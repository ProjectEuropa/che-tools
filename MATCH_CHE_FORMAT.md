...
### OKEサマリー（556B）内部構造メモ（L3AR.CHEで観測）

OKEブロック内の `+0x1C94` から始まる 556バイトは、単なる「名前」だけではなく、ある程度まとまったデータ構造を持っている。
L3AR.CHEの実体入りOKEでサンプルを見た限り、以下のように「OKEプログラム先頭側のヘッダー領域」に近い並びに見える（確定ではなく仮説）。

#### 検証: team.CHEのOKE先頭556Bと完全一致

`team.CHE`（CETD）の OKE1（0x374開始）先頭 556バイトが、CEMD側のOKEブロックサマリー（`block+0x1C94` の556バイト）と **バイト完全一致** することを確認した。

- `team.CHE` OKE1名: `アトラスＬＴＳ２`（OKE2/3は 0xCD パディングで実体なし）
- 一致例:
  - `test_converted.CHE` の Block 0 サマリー（スロットから参照される）＝ `team.CHE` OKE1先頭556B
  - `L3AR.CHE` の Block 18 サマリー（本ファイルでは未参照）＝ `team.CHE` OKE1先頭556B

このため、OKEサマリー556Bは「表示用サマリー」として独立に設計されているというより、**team.CHEのOKEプログラム先頭部（ヘッダ/アイコン等）を切り出したもの**と考えるのが自然。

#### 型推定（候補）

556バイトは OKEプログラム先頭 `0x000-0x22B` に相当し、既存の区切り（`0x018/0x040/0x080/0x0FC/0x140`）で概ね説明できる。
ただし、文字列領域のパディングが必ずしも 0 で埋まらず、名前のNULL終端後に非ゼロ値（例: `0x0076` の繰り返し）が残る個体があるため、読み取り時は「固定長+NULL終端」の扱いが安全。

特に `0x018` 付近は u32（リトルエンディアン）として見たときに取り得る値の種類が少なく、フラグ/初期値領域の可能性がある（L3AR.CHEの実体入りOKE 28件で観測）:

| オフセット | 型(候補) | 観測されやすい値（例） |
|---:|---|---|
| 0x018 | u32 | `0xFFFFFFFF`, `0x00000000`, `0xFFFFFF00`, `0xCDCDCD00` |
| 0x01C | u32 | `0` が多数、まれに `1` |
| 0x020 | u32 | `0xFFFFFF00` または `0` が多い（他に少数の固定値） |
| 0x024 | u32 | `0xFFFFFFFF` または `0` が多い |
| 0x028 | u32 | `0xFFFFFFFF` または `0` が多い |

`0x040-0x07F` は個体によって 0/`0xCCCCCCCC`/`0x44444444` などの埋めが目立つものがあり、数値テーブルというより「パターン/マスク」のようなデータに見える（現時点では用途不明）。

| サマリー内オフセット | サイズ | 観測内容 |
|---|---:|---|
| +0x000 | 24 | OKE名（Shift-JIS、NULL終端/パディング） |
| +0x018 | 0xE4 (228) | パラメータ/不明領域（0/0xFF/0xCDのパディングや、小さな数値列が混在） |
| +0x0FC | 68 | 「アイコン/パターン」っぽい領域（0x44/0x33/0xCC等の繰り返しが多い個体がある） |
| +0x140 | 0xEC (236) | 不明領域2（個体によっては 0x44 や 0x33 で大きく埋まる） |

観測例（L3AR.CHE、サマリー内 +0x0FC〜の68バイト）:
- `夜霧` は 0x44 が支配的（塗りつぶしに見える）
- `Ｏｃｕｌａｒ` は 0x33 が支配的
- `脱兎・承Ａ` は 68バイトが全ゼロ

注意:
- ここで言う「アイコン/パターン」は見た目がビットマップに近いという意味で、寸法/bppは未確定。ただし 68バイトは `16x17` ピクセルの `2bpp`（272px×2bit=68B）と整合するため、有力候補。
- 「OKEサマリー＝team.CHEのOKEプログラム先頭556B」は、少なくとも `team.CHE(アトラスＬＴＳ２)` と `test_converted.CHE` / `L3AR.CHE` の該当OKEでバイト一致を確認済み（上記参照）。
...
**OKEアイコンビットマップ (+0x0FC):**
- 実データは0x44/0x33/0xCC等の繰り返しが多く、ビットマップに見えるが、寸法やbppは未確定（サイズ68バイトと「16x16 4bpp」は整合しないため要再検証）。
- パレットインデックス値（らしき）パターンが現れる個体がある（例: 0x33主体、0x44主体）。
...
| 0x0B8-0x147 | なし | **OKEサマリーはテンプレート依存（重要）** |
...
補足（テンプレート運用）:
- `web-app/template.CHE`（SML.CHE系）では、スロットのOKEインデックスが `0xFFFFFFFF` や「空ブロック参照（サマリー先頭0）」になっている場合がある。
- team.CHE→match.CHE生成では、スロットOKEサマリー `+0x00` が「有効なOKEブロック（例: Block 0-2）」を参照するように補正しないと、ゲーム/ツール側でOKEが空扱いになる可能性がある。
...
## 実ファイル検証メモ: match.CHE（このリポジトリ直下の match.CHE）

このセクションは、実ファイル `match.CHE` を読んでドキュメント記載のオフセットが妥当かを確認したメモ。

### ヘッダー値（抜粋）
- 0x018 / 0x168 大会名: `小ＣＰＵ大会正順`
- 0x034 / 0x184 チーム数: 15
- 0x038 / 0x188 マッチ数: 22
- 0x120 フィールド: 3（上級演習所）
- 0x124 方式: 12（リーグ戦）
...
python3 - <<'PY'
import struct
b=open('match.CHE','rb').read()
u32=lambda o: struct.unpack_from('<I', b, o)[0]
print('team', u32(0x34), 'match', u32(0x38), 'field', u32(0x120), 'mode', u32(0x124))
PY
```

### スロットの実態
- スロットは常に16個存在するが、`チーム数=15` の場合は末尾スロットが未使用になり得る。
- 本ファイルでは Slot 16（0-basedでslot15）が未使用で、`チームID=0xFFFFFFFF` / OKEインデックス3つとも `0xFFFFFFFF`。
- 未使用スロットの文字列領域が 0xCD パディング混在になっており、Shift-JISとして読むと `ﾍ` のようなゴミ文字に見えることがある（例: owner先頭が 0xCD で、NULL終端が直後にある）。

### OKEブロック突き合わせ（インデックス参照先）

スロット内 OKEサマリーの `+0x00` は、OKEブロック領域（`0x38BC + idx*7872`）への 0-based インデックス参照。
本ファイルのスロットが参照しているOKEインデックスは `0-8` のみだった（`0xFFFFFFFF` は未使用）。

OKE名は OKEブロック内サマリー領域（`block + 0x1C94`）先頭24バイトをShift-JISとして復号した値。

#### 参照されていたOKEブロック（0-8）

| idx | OKE名（block+0x1C94） |
|-----|------------------------|
| 0 | イカヅチＡＫ２ |
| 1 | ＮＥＷゲイザー |
| 2 | モッキンサクラデ |
...
#### スロット→OKE参照の例（チーム名 → [OKE1, OKE2, OKE3]）

| Slot(0-based) | チーム名 | idx | OKE名 |
|---:|---|---|---|
| 0 | ジョートアモンＡ | [4, 3, 5] | [サクグリフM, サクグリフS, サクグリフR] |
| 3 | 早すぎチェリー木琴 | [7, 6, 8] | [ラジカル１０Ｓ, アトラス１３Ｓ, ３６] |
| 7 | 赫の世界 | [1, 0, 2] | [ＮＥＷゲイザー, イカヅチＡＫ２, モッキンサクラデ] |
| 14 | Cyanopica | [7, 6, 8] | [ラジカル１０Ｓ, アトラス１３Ｓ, ３６] |

補足:
- 同一OKEブロックは複数スロットから共有参照される（例: idx 7/6/8 は多数のチームが共通で参照）。
- OKEブロック 22-30 は同一内容（SHA-1一致）で、OKE名が空・サマリーが 0xCD パディング主体の「未使用っぽい」ブロックだった（本ファイルでは参照されていない）。

### 横断解析メモ（複数CEMDを比較）

#### 重複ヘッダー（0x184/0x188）は未初期化のファイルがある
`0x034/0x038` と `0x184/0x188` が常に一致するとは限らない。特に `0x184/0x188` が `0` や `0xCDCDCDCD` のままのファイルが混在していた。
アプリ側で生成・更新する場合は両方を書き換える前提でよいが、解析・読み込み時は `0x034/0x038` を正として扱うのが安全。

#### OKEサマリーの magic/flag も未初期化のパターンがある
スロット内 OKEサマリー（48B）の `+0x04` / `+0x08` は多くのファイルで `0x08ACE7C8` / `1` だが、`0` や `0xCDCDCDCD` のままのものもあった。
（インデックス `+0x00` は 0-30 / 未使用は `0xFFFFFFFF` の範囲内で、範囲外参照は観測されなかった）

#### “空ブロック”は複数の典型パターンがある
OKEブロック（7872B）はファイルによって「実体が入っていない」ブロックが多数ある場合がある。観測できた代表例:
- **全ゼロブロック**（SHA-1例: `385f21dff5`）: 31ブロック全部がこれで埋まっているファイルもあった
- **0xCD主体ブロック**（SHA-1例: `9909a36a0d`）: パレットに固定値があり、サマリー先頭が `00 CD CD ...` のような形（SML.CHEでは Block 4-30 がこれに近い）
- **ほぼゼロブロック**（SHA-1例: `7ed1084fc6`）: パレット/サマリーがゼロで、わずかに非ゼロが残る

このため、OKE名（`block+0x1C94` 先頭24BをShift-JIS復号）は「実体があるファイルでは有効」だが、「空ブロックが多いテンプレート」では空文字になり得る。

### 未解明領域の実値（参考）
- 0x3888-0x38BB（52バイト）は全てゼロ（本ファイル）。
- 0x40FE0-0x40FE3 は `65 81 53 cf`（floatとしては `-3548472576.0`）。用途は未特定。
...
### ~~OKEプログラムの格納方法（重要な発見）~~ [廃止: 2025-12-09]

**⚠️ 注意: 以下の「2ブロックペア」仮説は誤りでした。正しくは「1ブロック=1OKE」です（更新履歴参照）**

<details>
<summary>古い情報（参考用）</summary>

**~~OKEプログラムは2ブロックにまたがって格納される~~：** （誤り）
...
**正しい情報:**
- 1ブロック = 1 OKE（独立した7,872バイト）
- match.CHEのOKEブロックにはAIプログラムコード全体は含まれない
- 表示用のサマリー情報（OKE名、パレット、ビットマップ）のみ格納
- 詳細は上記「OKEブロック構造」セクション参照
...
⚠️ 注意: この節は「2ブロックペア」仮説に基づく過去の検証ログで、2025-12-09の修正（1ブロック=1OKE、match.CHEはAIコード非搭載）以降の結論とは整合しない。
